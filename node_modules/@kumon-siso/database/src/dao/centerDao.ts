...
export class CenterDao {
  ...
  async getCenterInfo(): Promise<CenterInfo | undefined> {
    return this.db.get<CenterInfo>('SELECT * FROM center_info WHERE id = 1');
  }
  ...
  async initializeCenter(data: {
    ...
    if (existing) {
      ...
      await this.db.run(
        `INSERT INTO center_info (id, center_name, contact_phone, time_zone, staff_pin_hash, logo_url, accent_color, is_onboarded)
         VALUES (1, ?, ?, ?, ?, ?, ?, 1)`,
        [
          data.center_name,
          data.contact_phone,
  ...
  async updateSettings(settings: Partial<CenterInfo>): Promise<void> {
