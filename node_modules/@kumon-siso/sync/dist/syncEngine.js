...
class SyncEngine {
    ...
    smsDao;
    auditDao;
    constructor(db, parentRelayBaseUrl = 'http://localhost:3000') {
        this.parentRelayBaseUrl = parentRelayBaseUrl;
        this.attendanceDao = new database_1.AttendanceDao(db);
        this.studentDao = new database_1.StudentDao(db);
        this.smsDao = new database_1.SmsDao(db);
        this.auditDao = new database_1.AuditDao(db);
    }
        ...
        for (const evt of payload.events) {
            try {
                ...
                switch (evt.event_type) {
                    case shared_1.EVENT_TYPES.STUDENT_CHECKED_IN: {
                        ...
                        await this.attendanceDao.logEvent({
                            eventId: evt.id,
                            sessionId,
                            eventType: shared_1.EVENT_TYPES.STUDENT_CHECKED_IN,
                    ...
                    case shared_1.EVENT_TYPES.STUDENT_CHECKED_OUT: {
                        await this.attendanceDao.checkoutStudent(evt.attendance_session_id, evt.originating_device_id, evt.client_timestamp);
                        await this.attendanceDao.logEvent({
                            eventId: evt.id,
                            sessionId: evt.attendance_session_id,
                            eventType: shared_1.EVENT_TYPES.STUDENT_CHECKED_OUT,
                            originatingDeviceId: evt.originating_device_id,
                            clientTimestamp: evt.client_timestamp,
                            idempotencyKey: evt.idempotency_key,
                            isOverride: evt.is_override,
                    ...
                    case shared_1.EVENT_TYPES.PICKUP_OVERRIDE: {
                        await this.attendanceDao.checkoutStudentWithOverride(evt.attendance_session_id, evt.originating_device_id, evt.client_timestamp, evt.override_reason || 'Staff Pickup Override');
                        await this.attendanceDao.logEvent({
                            eventId: evt.id,
                            sessionId: evt.attendance_session_id,
                            eventType: shared_1.EVENT_TYPES.PICKUP_OVERRIDE,
                            originatingDeviceId: evt.originating_device_id,
                            clientTimestamp: evt.client_timestamp,
                            idempotencyKey: evt.idempotency_key,
                            isOverride: true,
                    ...
                    case shared_1.EVENT_TYPES.DROPOFF_OVERRIDE: {
                        await this.attendanceDao.recordDropoffOverride(evt.attendance_session_id, evt.override_reason || 'Staff Drop-off Override');
                        await this.attendanceDao.logEvent({
                            eventId: evt.id,
                            sessionId: evt.attendance_session_id,
                            eventType: shared_1.EVENT_TYPES.DROPOFF_OVERRIDE,
                            originatingDeviceId: evt.originating_device_id,
                            clientTimestamp: evt.client_timestamp,
                            idempotencyKey: evt.idempotency_key,
                            isOverride: true,
    ...
    async queueParentAckSms(sessionId, student, timeIn) {
        // Generate secure parent acknowledgment link token
        const rawToken = (0, shared_1.generateSecureToken)(32);
        ...
        if (student.parent1_phone) {
            await this.smsDao.queueSms({
                ...
                recipientMasked: (0, shared_1.maskPhoneNumber)(student.parent1_phone),
                parentIndex: 1,
                messageBody: msgBody,
            });
        }
        ...
        if (student.parent2_phone && student.parent2_phone !== student.parent1_phone) {
            await this.smsDao.queueSms({
                ...
                recipientMasked: (0, shared_1.maskPhoneNumber)(student.parent2_phone),
                parentIndex: 2,
                messageBody: msgBody,
            });
        }
